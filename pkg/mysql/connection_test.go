...
func TestReadHandshakeResponse(t *testing.T) {
	...
	fmt.Printf("attrs: %v\n", h.attrs)
	expectedAttrs := map[string]string{"_client_version": "0.9.3", "_pid": "48253", "_client_name": "pymysql"}
	if !reflect.DeepEqual(h.attrs, expectedAttrs) {
		t.Fatalf("h.attrs mismatch, expected: %s, got: %s\n", expectedAttrs, h.attrs)
	}
